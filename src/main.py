...
@app.route('/api/search_asset
', methods=[
'GET
'])
def search_asset_api():
    symbol_input = request.args.get(
'symbol
')
    if not symbol_input:
        return jsonify({
'error
': 
'Símbolo do ativo não fornecido
'}), 400
...
'.
' in symbol_to_query:
        market_suffix = symbol_to_query.split(
'.
')[-1]
        if market_suffix == 
'SA
...
'] = query_region_br
    ...
    try:
        ...
        asset_info = {}
        if chart_data and chart_data.get(
'chart
') and chart_data[
'chart
'].get(
...
', [])
            ...
            asset_info[
'volume
'] = last_valid_volume if last_valid_volume is not None else 
'N/A
'
        else:
            error_details = chart_data.get(
'chart
', {}).get(
'error
')
...
'error
': f
"Não foi possível obter dados do gráfico para o símbolo: {symbol_input}. Detalhes: {error_description}"
}), 404
        return jsonify(asset_info)
    except requests.exceptions.HTTPError as http_err:
        error_message = f
'Erro HTTP ao buscar dados do ativo: {http_err}
'
        try:
...
'):
                error_message = f
"Erro da API Finance: {error_data['chart']['error']['description']}"
        except ValueError:
            pass
        return jsonify({
'error
...
'error
': f
'Erro de requisição ao buscar dados do ativo: {req_err}
'}), 500
...
'error
': 
'Erro interno ao processar dados do ativo.
', 
'details
': str(e)}), 500
...
')
def serve(path):
    # Serve static files like index.html, or specific paths if they are not template-based routes
    if path != "" and os.path.exists(os.path.join(app.static_folder, path)):
        return send_from_directory(app.static_folder, path)
    # Serve index.html as the default page
    index_path = os.path.join(app.static_folder, 
'index.html
')
...
'index.html
')
    # If trying to access a route like /login directly and it's handled by render_template, this part might not be hit
    # Fallback if no specific file or index.html is found in static, though usually handled by other routes.
